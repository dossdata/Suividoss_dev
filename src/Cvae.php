...
class Cvae extends Connection{
    public function insertCvae($id_situation,$montant,$date_cvae_karlit,$date_envoie_client,$date_de_validation_client,
    $date_teletransmission,$date_validation_edi,$commentaire,$montant_cvae,$utilisateur_id){
       ...
       if(empty($date_validation_edi)){$date_validation_edi = null;}
            if( count($this->recuereexistid($id_situation)) < 1){
                $sql = "INSERT INTO
                 suividossdb.cvae(id_situation,montant,date_cvae_karlit,date_envoie_client,date_de_validation_client,date_teletransmission
                ...
                $sql = "UPDATE suividossdb.cvae SET 
                ...
                ,date_de_validation_client=:date_de_validation_client,date_teletransmission=:date_teletransmission
                 ,date_validation_edi=:date_validation_edi,montant_cvae=:montant_cvae,commentaire=:commentaire
   ...
   public function recuereexistid($id_situation){
		$sql = "SELECT * FROM suividossdb.cvae  where id_situation =:id_situation";
		$res = $this->Getconnexion()->prepare($sql);
